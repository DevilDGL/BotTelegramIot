...
bot = telebot.TeleBot("AquiVaElToken")
...
def send_welcome(message):
	...
	global encendido
	if not(encendido):
		bot.reply_to(message, f"Led encendido")
		encendido = True
	else:
		bot.reply_to(message, f"Led apagado")
		encendido = False
...
def send_welcome(message):
	...
	if not(txt):
		bot.reply_to(message, f"Error en el comando, deberia tener la forma:\n/r <Numero de dados>d<Numero de caras>\nEjemplo:\n/r 3d6", parse_mode= 'Markdown')
		exit()
	n1, n2 = txt.split("d")
	tiradas = sorted(np.random.randint(1,int(n2)+1,size = int(n1)),reverse= True)
	salida = "La tirada ha sido:\n"+str(tiradas)[1:-1]+"\nY su suma es:\n"+str(sum(tiradas))
	bot.reply_to(message, salida, parse_mode= 'Markdown')
